...
```{r}
mental_health_na <- subset(brfss2013$menthlth,!is.na(brfss2013$menthlth))
```

```{r}
physical_health_na <- subset(brfss2013$physhlth,!is.na(brfss2013$physhlth))
```


```{r}
bucket<-list(physical_health = physical_health_na, mental_health = mental_health_na)
```


```{r}
ggplot(melt(bucket), aes(value, fill = L1)) + geom_histogram(position = "stack", binwidth=0.5) + xlim(0,30) + coord_cartesian(ylim=c(0,1))
```
