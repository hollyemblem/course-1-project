...
```{r}
ggplot(data=subset(brfss2013, !is.na(menthlth)), aes(x=menthlth)) + geom_histogram(binwidth=0.5) + xlim(0, 50) + ylim(0,10000)
```
...
```{r}
ggplot(data=subset(brfss2013, !is.na(physhlth)), aes(x=physhlth)) + geom_histogram(binwidth=0.5) + xlim(0, 50) + ylim(0,10000)
```
...
Below Physical Health Visualisation
```{r}
ggplot(data=subset(brfss2013,income_median=="Below" & !is.na(physhlth)), aes(x=physhlth)) + geom_histogram(binwidth=0.5) + xlim(0,50) + ylim(0,10000)
```

Above Physical Health Visualisation

```{r}
ggplot(data=subset(brfss2013,income_median=="Above" & !is.na(physhlth)), aes(x=physhlth)) + geom_histogram(binwidth=0.5) + xlim(0,50) + ylim(0,10000)
```

Below Mental Health Visualisation

```{r}
ggplot(data=subset(brfss2013,income_median=="Below" & !is.na(menthlth)), aes(x=menthlth)) + geom_histogram(binwidth=0.5) + xlim(0,50)  + ylim(0,10000)
```

Above Mental Health Visualisation
```{r}
ggplot(data=subset(brfss2013,income_median=="Above" & !is.na(menthlth)), aes(x=menthlth)) + geom_histogram(binwidth=0.5) + xlim(0,50) + ylim(0,10000)
```

These findings show that amongst those below the median income in the United States, "not good" mental and physical health days are spread across a range, with a bimodal distribution. For those above the median, the data is still right-skewed, but is unimodal.
